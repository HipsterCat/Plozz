...
def validate_reference(value: Any, label: str) -> dict[str, str]:
    ...
    data = policy.reference(value)
    return {"path": value["path"], "sha256": policy.digest(data)}
...
def parse_release_record(
    ...
    *,
    ...
    checked_evidence = [validate_reference(item, "release evidence") for item in evidence]
    ...
    for target in targets:
        ...
        for item in checked_evidence:
            evidence_path = policy.physical_path(item["path"])
            if path_within(evidence_path, target_path):
                fail(f"release evidence is inside the deletion target: {evidence_path}")
        if path_within(path, target_path):
            fail(f"owner release record is inside the deletion target: {path}")
...
def inventory(
    ...
    *,
    ...
    for release_path in release_records:
        release_path = policy.physical_path(str(release_path))
        release, data = parse_release_record(
            release_path,
            minimum_seconds=minimum_seconds,
            ...
            targets.append(
                {
                    ...
                    "release_record": reference_for(release_path, data),
    ...
    return {"schema": SCHEMA, "scope": SCOPE, "targets": targets}
...
def validate_manifest_target(
    ...
    *,
    ...
    release_record = validate_reference(target["release_record"], "owner release record")
    evidence = target["release_evidence"]
    if not isinstance(evidence, list) or not evidence:
        fail("manifest target requires release evidence")
    checked_evidence = [
        validate_reference(item, "manifest release evidence") for item in evidence
    ]
    ...
    release_path = policy.physical_path(release_record["path"])
    release, _ = parse_release_record(
        release_path,
        minimum_seconds=minimum_seconds,
        current_time=current_time,
    )
    ...
    if expected_release_target not in release["targets"]:
    ...
    validate_target_location(kind, target_path, Path(worktree["path"]))
    for item in checked_evidence + [release_record]:
        protected = policy.physical_path(item["path"])
        if path_within(protected, target_path):
            fail(f"protected evidence is inside cleanup target: {protected}")
...
def validate_disjoint_targets(targets: list[dict[str, Any]]) -> None:
    roots = [Path(target["path"]) for target in targets]
    for index, root in enumerate(roots):
        for other in roots[index + 1:]:
            if path_within(root, other) or path_within(other, root):
                fail("overlapping cleanup targets are not permitted")
        for target in targets:
            for ref in [target["release_record"], *target["release_evidence"]]:
                if path_within(Path(ref["path"]), root):
    ...
    def inspect(value):
        if isinstance(value, dict):
            if set(value) == {"path", "sha256"}:
                ...
                if any(path_within(path, Path(t["path"])) for t in manifest["targets"]):
...
class RuntimeGuard:
    ...
    def check(self) -> None:
        ...
        for target in self.manifest["targets"]:
            ...
            validate_reference(target["release_record"], "owner release record")
            for evidence in target["release_evidence"]:
                validate_reference(evidence, "owner release evidence")
...
class TargetRemoval:
    def __init__(self, root: Path, entries: list[dict], guard, journal, opened):
        self.root, self.guard, self.journal, self.opened = root, guard, journal, opened
    ...
    def remove(self, path: Path, parent_fd: int) -> None:
        ...
        if expected["kind"] == "directory":
            ...
            try:
                ...
                intended = sorted(
                    Path(name).name for name in self.expected
                    if name != "." and (self.root / name).parent == path
                )
                if names != intended:
        ...
        try:
            for target in manifest["targets"]:
                ...
                removal = TargetRemoval(
                    root, entries_by_path[str(root)], guard, journal, open_inventory
                )
    ...
    try:
        if args.command == "inventory":
            ...
            if args.output:
                ...
                write_private_new(args.output, encoded.encode("utf-8"))
                result: Any = {

...
final class LiveTVGuideSectionTests: XCTestCase {
    func testUnwatchedCatalogDoesNotInventRecentOrFavoriteSections() {
        let model = makeModel()
        XCTAssertEqual(model.guideChannels.map(\.id), model.visibleChannels.map(\.id))
        XCTAssertEqual(Set(model.guideChannels.map(\.section)), [.channels])
        XCTAssertEqual(model.guideChannels.filter(\.startsSection).map(\.id), ["0"])
    }
    ...
    func testRecentHistoryIsUniqueBoundedAndMostRecentFirst() {
        ...
        XCTAssertEqual(model.guideChannels.prefix(3).map(\.id), ["1", "3", "2"])
        XCTAssertEqual(model.guideChannels.filter { $0.section == .recent }.count, 3)
        XCTAssertTrue(model.guideChannels.contains { $0.id == "0" && $0.section == .channels })
    }

    func testRecentFavoritesAndRemainingChannelsNeverDuplicate() {
        let model = makeModel()
        ...
        XCTAssertEqual(model.guideChannels.map(\.id), ["2", "1", "0", "3", "4", "5", "6", "7"])
        ...
        XCTAssertEqual(Set(model.guideChannels.map(\.id)).count, model.channels.count)
        model.toggleFavorite("0")
        XCTAssertFalse(model.guideChannels.contains { $0.section == .favorites })
        XCTAssertEqual(model.guideChannels.first?.id, "2")
    }
    ...
    func testEveryGroupRespectsSearchCategorySourceAndGuideFilters() throws {
        ...
        model.category = "News"
        XCTAssertEqual(model.guideChannels.map(\.id), ["2", "0", "4", "6"])
        model.query = "Channel 2"
        XCTAssertEqual(model.guideChannels.map(\.id), ["2"])
        model.query = ""
        model.source = .plex
        XCTAssertEqual(model.guideChannels.map(\.id), ["4", "6"])
        ...
        XCTAssertEqual(model.guideChannels.map(\.id), ["6"])
        model.resetFilters()
        model.favoritesOnly = true
        XCTAssertEqual(model.guideChannels.map(\.id), ["1", "0"])
        XCTAssertEqual(model.recentChannelIDs, ["3", "2", "1"])
    }

    func testCatalogRefreshPrunesMissingRecentsAndKeepsStableRowIDs() throws {
        let model = makeModel()
        let before = Set(model.guideChannels.map(\.id))
        model.toggleFavorite("5")
        model.tune("5")
        model.recordWatched("5")
        XCTAssertEqual(Set(model.guideChannels.map(\.id)), before)
        XCTAssertEqual(model.guideChannels.first?.id, "5")
        try model.replaceChannels(model.channels.filter { $0.id != "5" })
        XCTAssertTrue(model.recentChannelIDs.isEmpty)
        XCTAssertFalse(model.guideChannels.contains { $0.id == "5" })
    ...
    func testTransportSnapshotDoesNotBounceAsRecentsReorder() {
        ...
        XCTAssertEqual(model.guideChannels.prefix(3).map(\.id), ["2", "1", "0"])
    ...
    func testLargeCatalogRetainsOneStableEntryPerChannel() {
        let model = LiveTVPrototypeModel(scenario: .noGuide, isLargeCatalog: true)
        let id = model.channels[4_999].id
        model.tune(id)
        model.recordWatched(id)
        XCTAssertEqual(model.guideChannels.first?.id, id)
        XCTAssertEqual(model.guideChannels.count, 5_000)
        XCTAssertEqual(Set(model.guideChannels.map(\.id)).count, 5_000)
        model.stop()
